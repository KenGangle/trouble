...
from os import curdir, sep #gets me the current directory
...
def respond(sock):
    ...
    if len(parts) > 1 and parts[0] == "GET":
        ...
        if sendFile == True:
            try:
                f = open(curdir + sep + sock.path)
                sock.send_response(200)
                sock.send_header("Content-type",mimetype)
                sock.end_headers()
                sock.wfile.write(f.read())
                f.close()

            except FileNotFoundError:
                sock.send_error(404,"File Not Found " sock.path)
